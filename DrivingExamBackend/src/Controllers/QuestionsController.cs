...
namespace DrivingExamBackend.Controllers
{
    ...
    public class QuestionsController : ControllerBase
    {
        ...
        [HttpGet("exam")]
        public async Task<ActionResult<List<QuestionDto>>> GetExamQuestions([FromQuery] Guid[] moduleGuids, [FromQuery] int count = 20)
        {
            if (moduleGuids == null || moduleGuids.Length == 0)
                return BadRequest("Keine Module ausgewählt.");

            // Alle Fragen aus den angegebenen Modulen holen
            var questions = await _db.Questions
                .Where(q => moduleGuids.Contains(q.Module.Guid))
                .Select(q => new QuestionDto(
                    q.Guid, q.Number, q.Text, q.Points, q.ImageUrl, q.Module.Guid, q.Topic.Guid,
                    q.Answers.Select(a => new AnswerDto(a.Guid, a.Text)).ToList()))
                .ToListAsync();
            ...
            if (questions.Count < count)
                return BadRequest("Nicht genügend Fragen vorhanden.");
